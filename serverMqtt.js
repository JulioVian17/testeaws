...
//-------------MQTT-----------------------------
/*
const mqttServer = `mqtt://${ipv4Address}`; 
const mqttOptions = {
    username: 'usuario',  // Se necessário, substitua pelo seu usuário MQTT
    password: 'senha',     // Se necessário, substitua pela sua senha MQTT
};
const client = mqtt.connect(mqttServer, mqttOptions);
client.on('connect', () => {
    console.log('Conectado ao broker MQTT');
    client.subscribe('fabrica/garrafas', (err) => {
        if (!err) {
            console.log('Inscrito no tópico fabrica/garrafas');
        } else {
...
*/
app.post('/api/garrafas', (req, res) => {
  const { bottleCount1, bottlesPerHourInstant1, bottleCount2, bottlesPerHourInstant2 } = req.body;

  // Processa os dados recebidos
  console.log('Dados recebidos:');
  console.log(`Garrafas Rotuladas: ${bottleCount1}`);
  console.log(`Velocidade Rotuladora: ${bottlesPerHourInstant1}`);
  console.log(`Garrafas Envasadas: ${bottleCount2}`);
  console.log(`Velocidade Envasadora: ${bottlesPerHourInstant2}`);
...
app.post('/salvar-dados-rapidos', async (req, res) => {
  ...
  try {
    const { dataHora, itemCode, operacao, ordemNumero } = req.body;
    var formData = {};
